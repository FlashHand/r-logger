...
var genDeviceID = () => {
  ...
  if (!(d && d.length === 32)) {
    d = (0, import_js_md5.default)((0, import_nanoid.nanoid)(32) + navigator.userAgent);
...
var RLogger = class {
  constructor(config) {
    ...
    this.client = import_axios.default.create(this.axiosConfig);
    this.loadQ();
  }
  //加载持久化队列
  loadQ() {
  ...
  send(customPayload, config, level = "info") {
    const defaultPayload = {
      _d: genDeviceID()
    };
    let payloadArr = Array.isArray(customPayload) ? customPayload.map((p) => Object.assign({}, defaultPayload, this.basicPayload, p, { _lv: level })) : [Object.assign({}, defaultPayload, this.basicPayload, customPayload, { _lv: level })];
    let payload;
    if (this.interceptor)
      payload = this.interceptor(payloadArr);
    console.log(payload);
    if (config?.immediate) {
      try {
...
var defaultLogger = new RLogger({
  endpoint: "",
  axiosConfig: {
    headers: { "Content-Type": "application/json" }
  }
});
var aliLogger = new RLogger({
  endpoint: "",
  axiosConfig: {
    headers: {
      Accept: "*/*",
...
aliLogger.setInterceptor((payload) => {
  return {
    __logs__: payload
  };
});
